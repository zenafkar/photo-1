...
const MarketplacePresets = () => {
  return (
    <section className="py-16 md:py-24 bg-background border-t border-surface-border relative overflow-hidden text-text">
      {/* Background Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[radial-gradient(circle,rgba(79,70,229,0.06)_0%,transparent_70%)] rounded-full pointer-events-none" />
      
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-xs font-mono text-cyan-400 mb-6 backdrop-blur-md">
            <Share2 className="w-3.5 h-3.5 text-cyan-400" />
            <span>OMNICHANNEL READY</span>
          </div>
          <h2 className="font-display text-3xl md:text-5xl font-black text-text mb-6 tracking-tight">Marketplace Export Engine</h2>
        ...
        <div className="flex flex-col md:flex-row justify-center items-center md:items-end gap-10 md:gap-14">
          {presets.map((preset) => (
            <div key={preset.name} className="flex flex-col items-center group cursor-pointer">
              <div 
                className="glass-card shadow-[0_8px_30px_rgba(0,0,0,0.5)] rounded-[24px] relative overflow-hidden flex items-center justify-center mb-6 group-hover:border-primary/50 group-hover:shadow-[0_0_40px_rgba(99,102,241,0.2)] transition-all duration-500 group-hover:-translate-y-2"
                style={{ width: preset.w, height: preset.h }}
              >
                <div className="absolute inset-0 bg-gradient-to-tr from-primary/10 via-cyan-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                ...
                <div className="absolute top-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20">
                  <span className="text-[9px] font-mono font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/50 px-2 py-0.5 rounded shadow-[0_0_10px_rgba(52,211,153,0.3)]">
                    OPTIMIZED
                  </span>
                </div>
              ...
              <h4 className="text-text font-extrabold text-lg mb-2 group-hover:text-primary transition-colors">{preset.name}</h4>
              <div className="flex items-center gap-2">
                <span className="text-sm font-mono font-bold text-cyan-400 bg-cyan-500/10 border border-cyan-500/30 px-4 py-1.5 rounded-full shadow-[0_0_10px_rgba(6,182,212,0.2)]">
                  {preset.aspect}
                </span>
