...
export default function PromptFeatureHighlight() {
  ...
  return (
    <section className="py-20 bg-background border-y border-surface-border overflow-hidden relative">
      {/* Background elements */}
      <div className="absolute top-0 right-0 w-1/2 h-full bg-[radial-gradient(ellipse_at_top_right,rgba(79,70,229,0.1),transparent_50%)] pointer-events-none"></div>
        ...
        <ScrollReveal>
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div className="order-2 lg:order-1">
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-surface border border-surface-border text-xs font-bold text-primary mb-6 shadow-sm">
                <Wand2 className="w-3.5 h-3.5" />
                <span>AI Prompt Studio</span>
              </div>
              <h2 className="font-display text-3xl md:text-5xl font-black text-text tracking-tight mb-6 leading-tight">
                Bebas Pusing Mikir <br />
                <motion.span 
                  animate={{ backgroundPosition: ["0% 50%", "100% 50%", "0% 50%"] }}
                  transition={{ duration: 4, ease: "easeInOut", repeat: Infinity }}
                  className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-cyan-400 via-purple-500 to-primary bg-[length:200%_auto] inline-block"
                >
                  Prompt Foto.
                </motion.span>
              </h2>
            ...
            <div className="order-1 lg:order-2 relative">
              <div className="absolute inset-0 bg-gradient-to-tr from-primary/20 to-cyan-500/20 rounded-3xl transform rotate-3 blur-2xl"></div>
              
              <div className="glass-card rounded-3xl shadow-2xl p-6 md:p-8 relative z-10 overflow-hidden transform hover:-translate-y-2 transition-transform duration-500">
                {/* Header mock */}
                ...
                <div className="space-y-3 mb-6 relative">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-bold text-text-muted">Prompt <span className="text-red-500">*</span></label>
                    ...
                    <motion.div 
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-bold text-xs transition-colors duration-300 ${isGenerating ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-gradient-to-r from-primary to-cyan-500 text-white shadow-[0_0_15px_rgba(79,70,229,0.3)]'}`}
                    >
                  ...
                  <div className="w-full h-40 p-4 bg-slate-950 border border-slate-800 rounded-xl text-sm font-medium shadow-inner relative overflow-hidden flex flex-col">
                    {!isGenerating && displayedText === "" ? (
                      <span className="text-slate-500">Deskripsikan foto yang Anda inginkan...</span>
                    ) : (
                      <div className="relative z-10 text-cyan-300 leading-relaxed font-mono text-[13px]">
                        <span className="text-cyan-500 mr-2">&gt;</span>
                        {displayedText}
                        {isGenerating && <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 0.8 }} className="inline-block w-2 h-4 bg-cyan-400 ml-1 translate-y-1"></motion.span>}
                      </div>
                    )}
                    
                    {/* Glowing effect inside textarea when generating */}
                    {isGenerating && (
                      <div className="absolute inset-0 bg-gradient-to-r from-transparent via-cyan-500/10 to-transparent w-[200%] animate-[shimmer_2s_infinite] pointer-events-none" style={{ transform: 'skewX(-20deg)' }}></div>
                    )}
                  </div>
                </div>
