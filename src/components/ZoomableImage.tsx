...
export default function ZoomableImage({ src, onClose }: ZoomableImageProps) {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-sm overflow-hidden animate-in fade-in duration-200"
      onClick={onClose}
    >
      <div 
        ref={containerRef}
      ...
      >
        <motion.img
          ...
          className={`max-w-[90vw] max-h-[90vh] object-contain rounded-lg shadow-2xl ${scale > 1 ? 'cursor-grab active:cursor-grabbing' : 'cursor-default'}`}
